...
public class MainViewModel : ViewModelBase
{
    ...
    private void AskAndOpen()
    {
        ...
        {
            Filter      = "スケジュールファイル (*.tdc;*.oat)|*.tdc;*.oat|すべてのファイル (*.*)|*.*",
            Title       = "スケジュールを開く",
            Multiselect = true,
        };
        if (dlg.ShowDialog() != true) return;
    ...
    private void SaveAs()
    {
        ...
        {
            Filter   = "Tottoto スケジュール (*.oat)|*.oat|tdChart 互換 (*.tdc)|*.tdc",
    ...
    private void OpenTodayScheduleWindow()
    {
        ...
        RegisterTodayVm(vm);
        var win = new Views.TodayScheduleWindow(vm) { Owner = Application.Current.MainWindow };
    ...
    public List<TaskRowViewModel> BuildAllScheduleRows()
    {
        var buffer = new List<TaskRowViewModel>();
        foreach (var entry in Schedules)
            BuildFlatListNoFilter(entry.Root, 0, buffer);
        return buffer;
    }
                                       ...
                                       IReadOnlyList<bool>? ancestorHasNext = null)
    {
        ...
        for (int i = 0; i < children.Count; i++)
        {
            ...
            if (child is ScheduleFolder folder && folder.IsExpanded)
            {
                ...
                BuildFlatListNoFilter(child, depth + 1, target, next);
            }
        }
    }
